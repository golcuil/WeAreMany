...
## R-044 Doc drift beyond presence checks
- Risk: backlinks ensure reachability but not content correctness.
- Mitigation: standardized backlink lines + must_exist vs audit_if_exists checks; periodic operator doc review.
- Owner: CTO + PM
- Status: OPEN

## R-045 Detail doc drift after consolidation
- Risk: detail docs may reintroduce duplicated checklists over time.
- Mitigation: link to canonicals for release/onboarding steps; periodic doc audits.
- Owner: CTO + PM
- Status: OPEN
...
## R-047 JSON schema drift for operator tools
- Risk: JSON output schema changes could break automation or hide issues.
- Mitigation: allowlisted JSON output + contract tests; require schema_version discipline.
- Owner: CTO + SecurityEngineer
- Status: OPEN
